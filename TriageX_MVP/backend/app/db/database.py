...
ASSESSMENTS_TABLE = settings.SUPABASE_ASSESSMENTS_TABLE
...
def log_assessment(
    ...
    # Helper function to convert "yes"/"no" strings or bools to bool
    def _to_bool(value):
        if value is None:
            return None
    ...
    # Convert medical history fields
    has_medical_conditions = _to_bool(form_data.get("has_medical_conditions"))
    has_medications = _to_bool(form_data.get("has_medications"))
    is_pregnant = _to_bool(form_data.get("is_pregnant"))
    is_trauma_related = _to_bool(form_data.get("is_trauma_related"))
    
    # Convert pregnancy_weeks to int if possible
    pregnancy_weeks = form_data.get("pregnancy_weeks")
    pregnancy_weeks_int = None
    if pregnancy_weeks:
        if isinstance(pregnancy_weeks, int):
            pregnancy_weeks_int = pregnancy_weeks
        elif isinstance(pregnancy_weeks, str) and pregnancy_weeks.strip().isdigit():
            pregnancy_weeks_int = int(pregnancy_weeks.strip())
    ...
    payload = {
        ...
        "medical_conditions_other": form_data.get("medical_conditions_other"),
        "has_medications": has_medications,
        "medications": form_data.get("medications", []),
        "medications_other": form_data.get("medications_other"),
        "is_pregnant": is_pregnant,
        "pregnancy_trimester": form_data.get("pregnancy_trimester"),
        "pregnancy_weeks": pregnancy_weeks_int,
        "is_trauma_related": is_trauma_related,
